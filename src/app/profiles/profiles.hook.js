...
export function useProfile() {
    ...
    try {
      ...
      throw new Error(
        err.response?.data?.message ||
          "Erro ao processar a ação de buscar usuario pelo username."
      );
    } finally {
  ...
  const getUserById = async (id) => {
    try {
      ...
      throw new Error(
        err.response?.data?.message ||
          "Erro ao processar a ação de buscar usuario pelo id."
      );
    } finally {
      loading.value = false; // Desativa o estado de carregamento.
    }
  ...
  const checkUsername = async (username) => {
    try {
      ...
      throw new Error(
        err.response?.data?.message ||
          "Erro ao processar a ação de verificar o nome de utilizador."
      );
    } finally {
      loading.value = false;
    }
  ...
  const followUser = async ({ followingId, followerId, tab }) => {
    try {
      loading.value = true;
      ...
      throw new Error(
        err.response?.data?.message || "Erro ao processar a ação de seguir."
      );
    } finally {
      loading.value = false;
    }
  ...
  const followProfile = async ({ userIdToFollow, isFollowBack = false }) => {
    try {
      loading.value = true;
      const response = await api.post("/users/follow-user", {
        userIdToFollow,
  ...
  const editProfile = async ({
    ...
    try {
      ...
      throw new Error(
        err.response?.data?.message || "Erro ao actualizar o perfil."
      );
    } finally {
      loading.value = false;
    }
