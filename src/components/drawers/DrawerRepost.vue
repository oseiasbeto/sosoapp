        ...
        <div class="fixed inset-0 flex items-center justify-center p-2 sm:p-4">
          ...
          shadow-2xl transition-all relative">
                ...
                <div class="p-4">
                    ...
                    <div class="space-y-4">
                        <button
                            @click="handleReply(drawer?.data?.originalPost, drawer?.data?.isPost, drawer?.data?.isViewPage)"
...
const handleReply = async (originalPost, isPost, isViewPage) => {
    await toggleRepost({ originalPost, isPost, isViewPage, }).then(() => {
        closeDrawer()
    })
}
